export const blogList = [
  {
    id: 1,
    title: "08/13是vmks第一期計畫的尾聲",
    category: "development",
    subCategory: ["frontend", "ui/ux", "design"],
    description:
      "經過數月的努力與團隊合作，我們的軟體開發計畫終於來到了一個重要的里程碑。從需求分析、設計、程式碼編寫到測試，每一步都是一次對專業技能與團隊合作的挑戰。回顧這段期間，雖然我們遇到了許多困難與挑戰，但也學到了很多。軟體不僅僅是程式碼，更重要的是解決問題與提供價值的能力。我們深知這只是開始，未來還有更多的可能性等著我們去探索，但在此刻，我們為我們所達成的感到自豪。謝謝每一位參與此計畫的夥伴，期待我們下一次的冒險！",
    authorName: "Rafael Hsieh",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://images.unsplash.com/photo-1500835556837-99ac94a94552?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8dHJhdmVsfGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60",
  },
  {
    id: 2,
    title: "吳老闆透過廣告提升ntupair的知名度",
    category: "entertainment",
    subCategory: ["vacation", "holidays", "sight seeing"],
    description:
      "「尋找真心的伴侶，或是純粹的友情，ntupair帶你找到那個專屬的連結。在這裡，每一段對話都可能開啟全新的旅程。不再等待，與ntupair一同探索真摯的人際網絡。讓每一次滑動都成為未來的美好回憶。」",
    authorName: "Po Chun Wu",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://images.unsplash.com/photo-1503220317375-aaad61436b1b?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTR8fHRyYXZlbHxlbnwwfHwwfHx8MA%3D%3D&auto=format&fit=crop&w=500&q=60",
  },
  {
    id: 3,
    title: "東京銀座街頭",
    category: "shopping",
    subCategory: ["e-commerce store", "clothing", "shopping store"],
    description:
      "東京的銀座街頭，繁華的霓虹燈下隱藏著精緻的日本工藝品。從手工和服到現代科技產品，每一次的購物體驗都充滿新鮮感。一邊品味綠茶，一邊探索這都市叢林的購物天堂。",
    authorName: "Rafael Hsieh",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://images.unsplash.com/photo-1666947670466-501622af0938?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8JUU2JTlEJUIxJUU0JUJBJUFDJUU5JThBJTgwJUU1JUJBJUE3fGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60",
  },
  {
    id: 4,
    title: "喜馬拉雅山的挑戰",
    category: "adventure",
    subCategory: ["adrenaline", "stay-fit", "lifestyle"],
    description:
      "尼泊爾的喜馬拉雅山脈，是每位冒險者的挑戰。從茂密的森林到雪白的山頂，每一步都考驗著意志。但當站在那山巔，俯瞰那壯闊的風景，所有的努力都變得值得。準備好踏上這趟冒險之旅了嗎？",
    authorName: "Chatgpt",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://images.unsplash.com/photo-1683273131715-bb4a0e167130?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8JUU1JTk2JTlDJUU5JUE2JUFDJUU2JThCJTg5JUU5JTlCJTg1JUU1JUIxJUIxfGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60",
  },
  {
    id: 5,
    title: "義大利麵品嘗技法",
    category: "cooking",
    subCategory: ["bbq", "food", "lifestyle"],
    description:
      "義大利麵不僅僅是一道料理，它是一門藝術。從選擇麵條的種類到調製醬料的技巧，每一步都充滿情感。今天，讓我帶領大家，進入這道美味的背後秘密，一同體驗烹飪的樂趣。",
    authorName: "Chatgpt",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://plus.unsplash.com/premium_photo-1673809798970-30c14cfd0ab6?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8cGFzdGF8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=500&q=60",
  },
  {
    id: 6,
    title: "Wandering in Paris",
    category: "entertainment",
    subCategory: ["beaches", "sea", "holidays"],
    description:
      "夏日的巴黎，不只是浪漫。漫步在塞納河畔，每一個角落都藏著一段歷史。從凱旋門到艾菲爾鐵塔，巴黎的每一寸土地都是旅人的夢想。下次，希望你也能與我一同，體驗這'光之城'的魅力。",
    authorName: "Cahtgpt",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8cGFyaXN8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=500&q=60",
  },
  {
    id: 7,
    title: "Art & Perception",
    category: "art",
    subCategory: ["skill", "design", "passion"],
    description:
      "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.",
    authorName: "John Doe",
    authorAvatar: "/assets/images/author.jpg",
    createdAt: "August 13, 2023",
    cover:
      "https://images.unsplash.com/flagged/photo-1572392640988-ba48d1a74457?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8YXJ0fGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60",
  },
];
